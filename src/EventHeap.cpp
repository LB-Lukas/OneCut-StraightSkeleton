...
namespace straight_skeleton {
    template <typename T>
    size_t EventHeap<T>::parent(size_t i) { return (i - 1) / 2; }

    template <typename T>
    size_t EventHeap<T>::left(size_t i) { return 2 * i + 1; }

    template <typename T>
    size_t EventHeap<T>::right(size_t i) { return 2 * i + 2; }
    ...
    void EventHeap<T>::print_heap() const {
        ...
        for (auto& val : heap) {
            std::cout << val << " " << weight_map.at(val)
                      << " " << index_map.at(val) << std::endl;
        }
        std::cout << std::endl;
    }
